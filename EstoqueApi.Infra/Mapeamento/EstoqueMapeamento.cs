...
namespace EstoqueApi.Infra.Mapeamento
{
	public class EstoqueMapeamento : IEntityTypeConfiguration<Estoque>
	{
		...
		{
			builder.ToTable("ESTOQUE").OwnsOne(n => n.InfoCompra).Property(p => p.PrecoUnidade).HasColumnName("PRECO_UNITARIO").HasPrecision(16, 4);
			...
			builder.HasOne(n => n.Produto).WithOne(n => n.Estoque).HasForeignKey<Estoque>(k => k.ProdutoId);
		}
	}
}
